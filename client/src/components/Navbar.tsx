...
export function Navbar({ links, center, fixed = false, children }: NavbarProps) {
  ...
  return (
    <>
      <ServerWakeBanner triggerKey={location.pathname} />
      <header
        className={cn(
          'left-0 right-0 top-0 z-50 border-b border-border bg-card px-6 shadow-[0_1px_4px_rgba(28,20,16,0.07)] dark:shadow-[0_1px_0_rgba(210,110,90,0.28),0_4px_32px_rgba(0,0,0,0.7)] md:px-12',
          fixed ? 'fixed z-100' : 'sticky',
        )}
      >
        <div className="grid h-[58px] grid-cols-[1fr_auto_1fr] items-center">
